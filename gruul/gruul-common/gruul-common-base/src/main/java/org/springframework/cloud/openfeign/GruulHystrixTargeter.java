...
/**
 * @Description: 注入默认的 fallbackFactory
 * @Author: alan
 * @Date: 2019/8/10 17:23
 */
@SuppressWarnings("unchecked")
class GruulHystrixTargeter implements Targeter {
	...
	public <T> T target(FeignClientFactoryBean factory, Feign.Builder feign, FeignContext context,
		...
		GruulFallbackFactory gruulFallbackFactory = new GruulFallbackFactory(target);
		return (T) builder.target(target, gruulFallbackFactory);
	}
	...
	private <T> T targetWithFallbackFactory(String feignClientName, FeignContext context,
		...
		FallbackFactory<? extends T> fallbackFactory = (FallbackFactory<? extends T>)
				getFromContext("fallbackFactory", feignClientName, context, fallbackFactoryClass,
		...
		if (!targetType.isAssignableFrom(beanType)) {
			throw new IllegalStateException(String.format(
					"Incompatible " + fallbackMechanism + " instance. Fallback/fallbackFactory of " +
							"type %s is not assignable to %s for feign client %s", beanType, targetType,
					feignClientName));
		}
